...
void renderUI(GLFWwindow *window, int fbW, int fbH, float aspect,
              Camera &camera, std::vector<Enemy> &enemies,
              std::vector<Tower> &placedTowers, Map &levelMap) {
  ...
  for (int t = 0; t < 3; ++t) {
    ...
    if (sel)
      ...
      dl->AddRect(rmin, rmax, IM_COL32(55, 62, 88, 220), 6.f * uiScale * 0.25f,
                  0, borderNorm);
  ...
  if (selectedPlacedTowerIndex != -1 &&
      selectedPlacedTowerIndex < (int)placedTowers.size() && !gameOver) {
    Tower &tw = placedTowers[selectedPlacedTowerIndex];
    int baseCost = (tw.type == 0) ? cfg.towerBasicCost
                                  : ((tw.type == 1) ? cfg.towerLaserCost
                                                    : cfg.towerMortarCost);
    float baseDamage = (tw.type == 0)
                           ? cfg.towerBasicDamage
                           : ((tw.type == 1) ? cfg.towerLaserDamageBase
                                             : cfg.towerMortarDamage);

    float currentDamage = baseDamage * (1.0f + tw.upgradeLevel * 1.0f);
    int upgradeCost = baseCost * 2;
    int totalSpent = baseCost;
    for (int lvl = 0; lvl < tw.upgradeLevel; lvl++)
      totalSpent += baseCost * 2;
    int sellRefund = (int)(totalSpent * 0.5f);

    const char *typeName = (tw.type == 0)
                               ? "Basic Tower"
                               : ((tw.type == 1) ? "Laser Tower" : "Mortar");

    ImGui::SetNextWindowPos(
        ImVec2(10.f * uiScale, (float)fbH - 200.f * uiScale), ImGuiCond_Always);
    ImGui::SetNextWindowSize(ImVec2(300.f * uiScale, 0.f));
    ImGui::Begin("TowerMenu", nullptr,
                 ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
                     ImGuiWindowFlags_NoMove |
                     ImGuiWindowFlags_AlwaysAutoResize |
                     ImGuiWindowFlags_NoCollapse);

    ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "%s (Lvl %d)", typeName,
                       tw.upgradeLevel + 1);
    ImGui::Separator();
